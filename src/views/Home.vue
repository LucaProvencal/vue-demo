...
export default {
  ...
  methods: {
    addShape(newShape) {
      ...
      if (!this.shapes.length) {  // in reality this would be async call to DB. we are emulating DB id creation here.
        newShape.id = 1           // if no shapes, give the new one id = 1
      } else {
        newShape.id = this.shapes.slice(-1)[0].id + 1 // otherwise, give new one last existing id + 1
      }
